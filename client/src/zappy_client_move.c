...
int			zappy_client_move_front(zappy_client_t *client)
{
	...
	if (r == 0)
	{
		r = zappy_client_transceive(client, advance_cmd, strlen(advance_cmd), zappy_rsp_ok);
	}
	if (r == 0)
	{
		int current_row = 0;
...
int			zappy_client_move(zappy_client_t *client, int relative_pos_target)
{
	...
	if (r == 0)
	{
		...
		while (client->relative_pos < relative_pos_target)
		{
